	...
	return (
		...
		>
			<img
				...
				loading="lazy"
				decoding="async"
				className="absolute inset-0 h-full w-full object-cover transition-transform duration-[600ms] ease-in-out group-hover:scale-[1.045] group-focus-within:scale-[1.045]"
			/>
			<div
			...
			/>
			<div
				className={`relative z-10 flex h-full flex-col justify-end ${big ? "p-6 md:p-8" : "p-5"}`}
			>
				<p className="mb-1.5 text-[11px] font-bold uppercase tracking-[0.15em] text-brand-500">
					{industry.tagline}
				</p>
				<h3
					className={`font-display font-bold leading-tight text-white ${big ? "text-[22px] md:text-[27px]" : "text-[18px]"}`}
				>
					{industry.name}
				</h3>
				<div className="hidden xl:grid transition-[grid-template-rows,opacity] duration-500 ease-out xl:max-h-0 xl:grid-rows-[0fr] xl:opacity-0 md:group-hover:max-h-44 xl:group-hover:grid-rows-[1fr] xl:group-hover:opacity-100 xl:group-focus-within:max-h-44 xl:group-focus-within:grid-rows-[1fr] xl:group-focus-within:opacity-100">
