...
export default function CandidateApplicationForm() {
	...
	async function submit(e: React.FormEvent) {
		...
		const cv = validateFile(files.cv, {
			accept: ["pdf", "doc", "docx"],
			maxMB: 5,
			required: true,
		});
		if (cv) fErrs.cv = cv;
		const pp = validateFile(files.passport, {
			accept: ["pdf", "jpg", "jpeg", "png", "webp"],
			maxMB: 5,
		});
		if (pp) fErrs.passport = pp;
		const ct = validateFile(files.certificates, {
			accept: ["pdf", "jpg", "jpeg", "png", "webp"],
			maxMB: 5,
		});
		...
		setSending(true);
		const res = await submitForm("candidate", {
			...values,
			cv: files.cv,
		...
		if (res.ok) setDone({ ref: res.ref, simulated: res.simulated });
			...
			setErrors({
				_submit: `Submission failed (${res.error}). Please try again.`,
			});
	}
		...
		return (
			...
			>
				...
				<h3 className="font-display text-[22px] font-extrabold text-navy">
					Application received — {done.ref}
				</h3>
				<p className="mx-auto mt-3 max-w-md text-[14px] leading-relaxed text-muted">
					Your profile enters our talent pool for matching verified employer
					requirements. Keep your reference number for follow-up.
				</p>
				<div className="mt-6 flex flex-col justify-center gap-2.5 sm:flex-row">
					<CTAButton to="/recruitment-process" variant="navy">
						See how selection works
					</CTAButton>
					<CTAButton to="/job-categories" variant="outline" withArrow={false}>
				...
				{done.simulated && (
					<p className="mx-auto mt-7 max-w-md border-t border-line pt-4 text-[11.5px] leading-relaxed text-[#93a4ad]">
						...
						in this static build submissions are validated client-side only.
					</p>
				)}
			</div>
		);
	...
	return (
		<form
			...
			onSubmit={submit}
			noValidate
			aria-label="Overseas jobs application form"
			className="card-x p-5 md:p-8"
		>
			<FormErrorSummary
			...
			<div className="grid gap-x-5 gap-y-4 sm:grid-cols-2">
				...
				>
					<Select
						...
						value={String(values.preferredCountry ?? "")}
						onChange={set("preferredCountry")}
						invalid={!!errors.preferredCountry}
					>
						...
						{[
							...
							"Israel",
							"Mauritius",
							"European market (specify in message)",
							"Any suitable market",
						].map((c) => (
				...
				<Field id="passportStatus" label="Passport status">
					<Select
						id="passportStatus"
						name="passportStatus"
						value={String(values.passportStatus ?? "")}
						onChange={set("passportStatus")}
					>
						<option value="">Select…</option>
				...
				<Field id="employmentStatus" label="Current employment status">
					<Select
						...
						value={String(values.employmentStatus ?? "")}
				...
				<Field
					id="expectedSalary"
					label="Expected salary (INR/month)"
					hint="Optional — helps us match employer bands."
				>
					<TextInput
						id="expectedSalary"
						name="expectedSalary"
						value={String(values.expectedSalary ?? "")}
				...
				<Field id="cv" label="CV / biodata" required error={fileErrors.cv}>
					<FileInput
						id="cv"
						name="cv"
						invalid={!!fileErrors.cv}
						onChange={(f) => setFiles((s) => ({ ...s, cv: f }))}
					/>
				</Field>
				<Field
					id="passport"
				...
				>
					<FileInput
						id="passport"
						name="passport"
						invalid={!!fileErrors.passport}
						onChange={(f) => setFiles((s) => ({ ...s, passport: f }))}
					/>
				</Field>
				<Field
					id="certificates"
					label="Certificates (optional)"
					error={fileErrors.certificates}
				>
					<FileInput
						id="certificates"
						name="certificates"
						invalid={!!fileErrors.certificates}
						onChange={(f) => setFiles((s) => ({ ...s, certificates: f }))}
					/>
				</Field>

				<Field
					id="message"
					label="Message to the recruitment team"
				...
				>
					<TextArea
						...
						placeholder="Certifications, visa history, notice period, anything relevant."
					/>
				</Field>
				...
				<div className="sm:col-span-2">
					<p className="mb-4 flex items-start gap-3 rounded-md border border-[#e8cf8f] bg-[#fdf6e3] p-4 text-[12.5px] leading-relaxed text-[#6d4e05]">
						<AlertTriangle
							className="mt-0.5 h-4 w-4 shrink-0 text-[#a57708]"
							aria-hidden="true"
						/>
						<span>
							<strong className="font-bold">Candidate safety notice.</strong>{" "}
							Verify official communication from Sanska before sharing sensitive
							documents or making any payment. No legitimate recruitment step
							requires you to hand over original passports as deposit.
						</span>
					</p>
				</div>
				...
				<div className="sm:col-span-2">
					...
					>
						I confirm my details are accurate and consent to Sanska
						International storing and assessing my profile for overseas job
						matching, per the{" "}
						<a
							href="/privacy"
			...
			<div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
				<p className="text-[12px] text-muted">
					Submitting does not guarantee interview calls or employment.
				</p>
