...
export async function submitForm(
	...
	const ref = makeRef(
		kind === "employer" ? "REQ" : kind === "candidate" ? "APP" : "MSG",
	);
	...
	for (const [k, v] of Object.entries(payload)) {
		...
		flat[k] =
			v instanceof File
				? `${v.name} (${Math.round(v.size / 1024)} KB) — file not uploaded in static mode`
				: String(v);
	}
	...
	console.info(
		`[sanska] ${kind} submission captured (no endpoint configured)`,
		flat,
	);
